...
void commanderSetSetpoint(setpoint_t *setpoint, int priority)
{
  ...
  if (priority >= currentPriority) {
    ...
    // === GRAVA A INCLINAÇÃO ATUAL (lu) ===
    lastRoll = setpoint->attitude.roll;
    lastPitch = setpoint->attitude.pitch;
...
void commanderGetSetpoint(setpoint_t *setpoint, const state_t *state)
{
  ...
  if ((currentTime - setpoint->timestamp) > COMMANDER_WDT_TIMEOUT_SHUTDOWN) {
    ...
    // === MANTÉM A ÚLTIMA INCLINAÇÃO ===
    setpoint->attitude.roll = lastRoll;
    setpoint->attitude.pitch = lastPitch;
    setpoint->thrust = 32000;
