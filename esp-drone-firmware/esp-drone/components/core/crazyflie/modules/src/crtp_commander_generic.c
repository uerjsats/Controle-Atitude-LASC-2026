...
// 1. Alocar memória suficiente (16 bytes) para a função de descompressão
  float temp_q[4];
  
  // 2. Descomprimir o valor para o array temporário seguro
  quatdecompress(values->quat, temp_q);
  
  // 3. Mapear os valores individualmente para garantir a integridade da struct
  setpoint->attitudeQuaternion.q0 = temp_q[0];
  setpoint->attitudeQuaternion.q1 = temp_q[1];
  setpoint->attitudeQuaternion.q2 = temp_q[2];
  setpoint->attitudeQuaternion.q3 = temp_q[3];

  // 4. Manter a configuração original dos modos de voo
